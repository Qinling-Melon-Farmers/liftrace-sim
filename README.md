...
2026-09-19新增：[当前高位/低位几何预筛入口](docs/CURRENT_PROFILE.md)，导入31–40冻结场景，导航全高障碍柱、固定航向相机、55cm已膨胀设计包络只计一次。[20组离线报告](docs/current_profile_20260919/REPORT.md)。此入口不预测识别/投递/完整Gate；下文M0/M2默认仍是历史研究模型。
...
## R64时间与辅助相机评审（2026-09-09）

[计划及10种策略×11布局离线结果](docs/r64_time_camera/PLAN.md)。本地fork已适配R64实录/刚性相机几何；本次只复用既有工具，未修改轻量算法或飞行。X/0.80m及Y/yaw90是候选，不能称最优飞行策略；辅助相机仅恢复方案评审，安全返航仍关闭。
